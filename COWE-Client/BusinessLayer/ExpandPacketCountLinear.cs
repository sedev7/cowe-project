...
{
    /// <summary>
    /// Expands packet counts into sequential units and uses linear extrapolation between cumulative probability data points to 
    /// to assign a cumulative probability to each unit in the sequence.
    /// </summary>
    public class ExpandPacketCountLinear : IExpandPacketCount
    {
        private BindingList<CumulativeProbabilityDistribution> _cpdPackets;
        private int _maxPacketCount;
        public ExpandPacketCountLinear(BindingList<CumulativeProbabilityDistribution> cpdPackets, int maxPacketCount)
        {
